...
func TestInitializeTimeouts(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			migrationTimeoutsEnv:   "1TiB=2h,3TiB=4h",
			maxMigrationTimeoutEnv: "",
			expectedTimeouts: map[int64]time.Duration{
				volumeSize2TB:                 migrationTimeoutSmall,
				volumeSize4TB:                 migrationTimeoutMedium,
		...
		},
		{
			...
			migrationTimeoutsEnv:   "500GiB=1h,1TiB=3h",
		...
		},
		{
			...
			migrationTimeoutsEnv:   "1TiB=2h=extra,3TiB",
			maxMigrationTimeoutEnv: "",
			expectedTimeouts: map[int64]time.Duration{
				volumeSize2TB: migrationTimeoutSmall,
				volumeSize4TB: migrationTimeoutMedium,
			...
			expectedMaxTimeout: 24 * time.Hour,
			expectWarnings:     true,
			warningMessages:    []string{"Invalid migration timeout format: 1TiB=2h=extra", "Invalid migration timeout format: 3TiB"},
		},
		{
			name:                   "invalid size format",
			migrationTimeoutsEnv:   "invalidSize=2h,1TiB=3h",
			maxMigrationTimeoutEnv: "",
			expectedTimeouts: map[int64]time.Duration{
				volumeSize2TB:             migrationTimeoutSmall,
				volumeSize4TB:             migrationTimeoutMedium,
		...
		},
		{
			...
			migrationTimeoutsEnv:   "1TiB=invalidDuration,2TiB=4h",
			maxMigrationTimeoutEnv: "",
			expectedTimeouts: map[int64]time.Duration{
				volumeSize4TB:                 migrationTimeoutMedium,
				2 * 1024 * 1024 * 1024 * 1024: 4 * time.Hour, // 2TiB
			},
		...
		},
		{
			...
			migrationTimeoutsEnv:   "1TiB=2h,invalidSize=3h,2TiB=invalidDuration,3TiB=6h",
...
func TestInitializeTimeoutsWithValidFormats(t *testing.T) {
	// Save original values
	originalMigrationTimeouts := make(map[int64]time.Duration)
	for k, v := range migrationTimeouts {
		originalMigrationTimeouts[k] = v
...
func TestInitializeTimeoutsWithDuplicatesAndOverrides(t *testing.T) {
	// Save original values
	originalMigrationTimeouts := make(map[int64]time.Duration)
	for k, v := range migrationTimeouts {
		originalMigrationTimeouts[k] = v
	...
	t.Run("test with duplicate sizes - last one wins", func(t *testing.T) {
		...
		// Set environment with duplicate sizes (1TiB appears twice with different timeouts)
		os.Setenv("MIGRATION_TIMEOUTS", "1TiB=1h,2TiB=3h,1TiB=2h,4TiB=6h")

		// Call initializeTimeouts
		initializeTimeouts()

		// Verify that the last timeout for 1TiB (2h) is used
		actualTimeout := getMigrationTimeout(512 * 1024 * 1024 * 1024) // 512GB, should use 1TiB timeout
		expectedTimeout := 2 * time.Hour                               // Last value for 1TiB
		assert.Equal(t, expectedTimeout, actualTimeout,
			"Expected last duplicate value (2h) to be used for 1TiB")
...
func TestMigrationTimeoutScenarios(t *testing.T) {
	...
	t.Run("migration timeout only - completes before max timeout", func(t *testing.T) {
		...
		diskClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
			func(ctx context.Context, resourceGroupName, diskName string) (*armcompute.Disk, error) {
				callCount++
